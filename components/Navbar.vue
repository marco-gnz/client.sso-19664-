      ...
      <b-navbar toggleable="lg" type="light" variant="faded" class="is-transparent is-fixed-top">
        ...
        <b-collapse id="nav-collapse" is-nav>
          <b-navbar-nav v-if="$auth.loggedIn">
            ...
            <b-nav-item class="font-weight-bold" href="#" to="/redes" v-if="$auth.user.permissions_roles.includes('ver-dato-maestro') || $auth.user.permissions.includes('ver-dato-maestro')">MANTENEDORES</b-nav-item>
            <b-nav-item class="font-weight-bold" href="#" to="/usuarios" v-if="$auth.user.permissions_roles.includes('ingresar-usuario') || $auth.user.permissions.includes('ingresar-usuario')">USUARIOS DEL SISTEMA</b-nav-item>
          </b-navbar-nav>
