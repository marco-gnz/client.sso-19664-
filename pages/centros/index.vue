      ...
      <div class="col-lg-9">
        <div class="card shadow">
          <div class="card-header py-3">
              <div class="row">
                  <div class="col-md-6">
                      <h6 class="m-0 font-weight-bold text-primary">Listado de centros formadores (Universidades)</h6>
                  </div>
                  <div class="col-md-6">
                      <button v-if="$auth.user.permissions_roles.includes('ingresar-dato-maestro') || $auth.user.permissions.includes('ingresar-dato-maestro')" v-b-modal.modal-add-centro class="btn btn-success float-right">Ingresar centro</button>
                      <ModalCentroFormador />
                      <ModalEditCentro />
                  </div>
              </div>
          ...
          <div class="card-body">
            ...
            <div class="row pt-lg-5">
              <template v-if="centrosFormadores.length">
                <div class="col-md-12">
                  ...
                  <table class="table pt-lg-2">
                    ...
                    <tbody>
                      <tr v-for="(centro, index) in centrosFormadores" :key="index">
                        <td>{{ (centro.cod_sirh != null) ? centro.cod_sirh : '--' }}</td>
                        <td>{{centro.nombre}}</td>
                        <td @click.stop="">
                          <el-dropdown>
                            <span class="el-dropdown-link">Acción<i class="el-icon-arrow-down el-icon--right"></i></span>
                            <el-dropdown-menu slot="dropdown">
                              <el-dropdown-item icon="el-icon-edit" @click.native="editCentro(centro)" v-b-modal.modal-edit-centro>Editar</el-dropdown-item>
                            </el-dropdown-menu>
                          </el-dropdown>
                        </td>
                      </tr>
                  ...
                  <nav aria-label="Page navigation example">
                    <ul class="pagination">
                      <li class="page-item" v-if="pagination.current_page > 1">
                        <button :disabled="centrosFormadores.length === 0" class="page-link" href="#" @click.prevent="changePage(pagination.current_page - 1)">
                            <span>Volver</span>
                        </button>
                      </li>
                      <li class="page-item" v-for="page in pagesNumber" :key="page" :class="page == pagination.current_page ? 'active' : '' &&  centrosFormadores.length ? 'active' : '' ">
                          <button :disabled="centrosFormadores.length === 0" class="page-link" href="#" @click.prevent="changePage(page)">
                              {{page}}
                          </button>
                      </li>
...
import ModalCentroFormador from '../../components/mantenedores/centroFormador/modal-centro-formador.vue';
import ModalEditCentro from '../../components/mantenedores/centroFormador/modal-edit-centro.vue';
export default {
  middleware: 'auth',
  ...
  methods:{
    ...mapMutations({
      pasingCentro: 'mantenedores/PASSING_CENTRO',
    ...
    editCentro(centro){
      this.pasingCentro(centro);
    },
    changePage(page) {
        this.currentPageAction(page);
         let object = { page: page };
        this.getCentrosFormadres(object);
    },
    searchCentros(){
    ...
    keySearchCentros() {
        ...
        this.setTimeoutBuscador = setTimeout(this.searchCentros, 500);
    },
  }
}
</script>
